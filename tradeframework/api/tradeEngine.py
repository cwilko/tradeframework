...
class TradeEngine:
    ...
    def getTradingInfo(self, dInfo, startCapital=1):
        ...
        returns = pd.DataFrame(dInfo.values.values, index=dInfo.values.index, columns=[
                               ["Capital", "Capital"], dInfo.values.columns])
        ...
        for l1 in mkts:
            ...
            b[0] = 0
            trade = pd.DataFrame(
                (a - b).reshape(len(dInfo.values), 2), index=ua[l1].index, columns=ua[l1].columns)
...
class DerivativeInfo(AssetInfo):
    ...
    def __init__(self, name, values, allocations,
                 weights, returns, childAssets):
        ...
        for l1 in range(assetCount):
            ...
            for l2 in uAllocations.columns.levels[0]:
                assetUAllocation = uAllocations[l2] * \
                    self.allocations[
                    self.allocations.columns.levels[0][l1]].values
