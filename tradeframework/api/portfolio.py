...
class Portfolio(Derivative):
    ...
    def handleData(self, context, assetInfo):
        ...
        # Update all children
        derivInfos = [derivative.handleData(
            context[self.name], assetInfo) for derivative in self.derivative_list]

        # Calculate portfolio allocation
        weights = self.optimizer.getWeights(
            context[self.name], [dInfo.returns for dInfo in derivInfos])
